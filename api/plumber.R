...
# -- INPUT VALIDATION
# =============================================================================
# The validation, auth-error and choice-list helpers live in ONE place:
# api/api_helpers.R, sourced above and also by every api/routes/*.R sub-router.
#
# This file used to re-define eleven of them immediately after sourcing that
# file, which silently overrode the shared versions for the endpoints defined
# here. The two copies had drifted apart (clean_text rejected "+" and ".";
# validate_foreman accepted "."), so the same query parameter validated
# differently depending on which file happened to define the endpoint.
# api_helpers.R now carries the union of both character sets.
...
#* Get active employee number → shortname lookup.
#* Returns a simple array of {emp_num, shortname} for all active employees.
#* Used by Google Sheets scripts to resolve typed employee numbers to names.
#* @get /v1/private/employees
#* @json
function(res) {
  tryCatch({
    con <- get_db_connection()
    ...
    list(
      ...
      data  = lapply(seq_len(nrow(rows)), function(i) {
        list(emp_num = rows$emp_num[i], shortname = rows$shortname[i])
      })
    )
...
#*                      Note: claims themselves expire after CLAIM_TTL (2 days,
#*                      see shared/redis_cache.R), so a larger window will not
#*                      surface anything older than that.
#* @get /v1/private/claims
#* @json
